...
// Random 8-hex-char IDs using crypto.getRandomValues with Math.random fallback for plain http contexts.
...
export interface DockerContainer { id: string; name: string; image: string; state: string; status: string; ports: string; protected: boolean }
...
// Per-resource editable config for hosting dialog; env = [[key, value], ...].
...
export interface PresetParam {
  key: string; env: string; default?: string | null; secret?: boolean; name?: string | null;
  // generate=false: a secret only the provider can hand out (installation key, API token) — never invented.
  generate?: boolean; hint?: string | null;
}
// One place the same app can be pulled from — the official image, a fork. `image` on the preset is the default.
export interface PresetSource { id: string; label: string; image: string; github?: string | null; note?: string | null; default?: boolean }
export interface ContainerPreset { id: string; label: string; group: string; image: string; port: number; icon?: string | null; description?: string | null; env?: string[][] | null; params?: PresetParam[] | null; companions?: PresetCompanion[] | null; volumes?: string[][] | null; bindMounts?: string[][] | null; files?: ExtraFile[] | null; args?: string[] | null; runtimeArgs?: string[] | null; tags?: string[] | null; gpu?: boolean; hostNetwork?: boolean; fixedPort?: boolean; website?: string | null; screenshots?: string[] | null; urlPath?: string | null; logo?: string | null; card?: string | null; github?: string | null; stars?: number | null; license?: string | null; language?: string | null; topics?: string[] | null; submitter?: string | null; source?: string | null; sources?: PresetSource[] | null; requires?: string[] | null }

// Existing resources satisfying a companion role + Aspire alternatives to avoid duplicates.
...
// Parameter resources on stack; candidates to reuse for preset secrets.
export function parameterCandidates(nodes: Node[]): Node[] {
  return nodes.filter(n => n.addMethod === "AddParameter");
}
// Nodes satisfying a companion role; candidates to reuse instead of a new one.
...
// Per-companion choice on drop: reuse, default container, or add Aspire resource.
...
export function presetParamDefault(p: PresetParam): string {
  return p.default ? p.default : p.secret && p.generate !== false ? randomSecret() : "";
}
...
// Builds nodes + edges a preset drops onto canvas; choices=reuse/new/add or "none" (app only).
...
// Reusable palette snippet; captured sub-graph the user saved.
export interface Snippet { id: string; name: string; group?: string | null; icon?: string | null; nodes: Node[]; edges: Edge[]; files: ExtraFile[] }

// Expands root node IDs into self-contained sub-graph; includes spawned nodes, parameters, & edges.
...
// Clones snippet's nodes+edges with fresh IDs, unique names, offset, & remapped references.
...
// Live per-resource data from running AppHost's Aspire resource service.
...
// Overlays live-resource snapshot onto stack graph; tracks top-level & orphan resources.
...
export function buildLiveOverlay(nodes: { id: string; resourceName: string }[], live: LiveResource[]): LiveOverlay {
  ...
  for (const r of visible) {
    ...
    if (nid) { statusByNodeId[nid] = r; continue; }
...
export function toLiteral(value: string, type: CatalogParam["type"], enumTypeName?: string | null): string {
  ...
  if (type === "resourceRef") return value;
  return `"${value.replace(/\\/g, "\\\\").replace(/"/g, '\\"')}"`;
}
// Builds `o => { o.Field = literal; }` lambda; returns "" when nothing set.
...
export function fromLiteral(literal: string): string {
  ...
  if (s.includes(".") && !/^[0-9.]+$/.test(s)) return s.slice(s.lastIndexOf(".") + 1);
  return s;
}
...
// String param naming filesystem location; offer server-side path picker.
...
// Graph-level lint; catches duplicates, port collisions, & dangling edges.
...
// Parses .env file into [key, value] pairs; skips blanks, strips export prefix.
...
// Finds file containing the CreateBuilder call (the AppHost entrypoint).
...
// Removes node & cascades: purges dangling references, rawStatements, & WithCall args.
...
// Node IDs whose center lies inside a boundary group's rectangle.
...
// Deps of node that would be orphaned by deleting it; owned=true if spawned by it.
...
// Stack-level run state shown per node; not per-resource Aspire health.
