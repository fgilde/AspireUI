...
public class TemplateTests : IClassFixture<TestWebAppFactory>
{
    ...
    {
        var templates = await _c.GetFromJsonAsync<List<TemplateInfo>>("/api/templates");
    ...
    {
        var res = await _c.PostAsync("/api/stacks/from-template/does-not-exist", null);
    ...
    {
        var res = await _c.PostAsync("/api/stacks/from-template/local-ai-demo", null);
