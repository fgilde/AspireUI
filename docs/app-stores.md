...
| **Unraid CA** | ✅ template URL by hand | ⏳ [submission form](https://ca.unraid.net/submit), then moderation |
| **CasaOS** | ✅ custom app source (zip URL) | ⏳ [PR #1032](https://github.com/IceWhaleTech/CasaOS-AppStore/pull/1032) open |
| **Cosmos** | ✅ compose import by hand | ⏳ [PR #295](https://github.com/azukaar/cosmos-servapps-official/pull/295) open |

The two PRs were opened from `store/casaos/` and `store/cosmos/` — the only difference in the
submitted copies is that asset URLs point at the receiving repository's CDN instead of ours.
...
**Official CasaOS store:** [PR #1032](https://github.com/IceWhaleTech/CasaOS-AppStore/pull/1032) is
open, adding `Apps/AspireUI/` with the compose file, `icon.svg`, `icon.png` (256×256), `thumbnail.png`
and three screenshots (1568×884 — the app store, the builder, the hosting view). Their store is on the
v2 protocol now, so the submission was validated the way their CI does it:

```bash
docker compose config -q          # in Apps/AspireUI
./scripts/build_dist.sh           # whole-store build
```

The build produces `dist/index.json` with `org.gilde.aspireui` (title, tagline, category *Developer*,
architectures, icon, thumbnail, version) and `dist/apps/org.gilde.aspireui/` with the per-architecture
compose files plus `meta.json` and `meta.de_DE.json` — the German `x-casaos` strings are picked up
automatically. To refresh the submission after a change here, copy the folder over again and repoint
the asset URLs to `https://cdn.jsdelivr.net/gh/IceWhaleTech/CasaOS-AppStore@main/Apps/AspireUI/…`.
...
**Official market:** [PR #295](https://github.com/azukaar/cosmos-servapps-official/pull/295) is open,
adding `servapps/AspireUI/` as it stands here. Their market index is generated by `index.js` scanning
the `servapps/` folders, so nothing else has to be registered; screenshots are picked up by file name
(`1.png`, `2.png`, `3.png`) and the icon URL in the `cosmos-icon` label already points at where the
file lives once merged.
