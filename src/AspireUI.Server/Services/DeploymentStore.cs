...
// Tracks hosting deployments (one per stack) in shared SQLite DB.
public class DeploymentStore
{
    private readonly string _connString;
    private readonly SqliteConnection? _keepAlive;
    private readonly Action<Deployment>? _onChanged;
    ...
    public void Upsert(Deployment d)
    {
        UsingConnection(conn =>
        {
            using var cmd = conn.CreateCommand();
    ...
    public void SetState(string id, string state, string? error = null)
    {
        UsingConnection(conn =>
        {
            using var cmd = conn.CreateCommand();
        ...
        if (_onChanged is not null) SafeNotify(Get(id));
    }
